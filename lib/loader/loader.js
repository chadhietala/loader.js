var loader, define, requireModule, require, requirejs, defineModule;
...
(function(global) {
  ...
  defineModule = function(name, modSpec) {
    if (arguments.length < 2) {
      unsupportedModule(arguments.length);
    }

    stringRegistry[name] = modSpec;
  };
  ...
  function findModule(name, referrer) {
    ...
    var stringMod = stringRegistry[name];

    if (stringMod) {
      var modSpec = JSON.parse(stringMod);

      if (modSpec.deps.length === 0) {
        return { module: { exports: new Function(modSpec.body)() } };
      } else {
        var reifiedDeps = modSpec.deps.map(function(dep) {
          if (dep === 'exports') {
            return {};
          }
          return findModule(dep, '(findModule)').module.exports;
        });

        var aliases = modSpec.deps.map(function(dep, i) {
          return 'var ' + dep + ' = arguments[' + i + '];';
        });
        var fn = new Function('{' + aliases.join('') + '\n' + modSpec.body + ' return exports;}');

        return {
          module: {
            exports: fn.apply(null, reifiedDeps)
          }
        };
      }
    }
